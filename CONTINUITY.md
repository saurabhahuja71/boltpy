...
- Local launchers: `~/.bashrc` now provides `bolt-s1` for Ollama on `:11435` and `bolt-s2` for SGLang on `:30002`; both run the backend ensure helper before `uv run`. TUI Enter submits; Shift+Enter inserts a newline.
